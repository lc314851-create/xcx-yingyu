# 给缺少分享能力的页面补 onShareAppMessage / onShareTimeline
import io, os
...
SHARE = {
    ...
    'quotes':   ('每日金句 · 补回落下的词', True),      # 已有 share，补 timeline
    'vocabtest':('测测你的词汇量', True),               # 已有 share，补 timeline
}

def block(page, title, path, timeline):
    s = ""
...
for page, (title, timeline) in SHARE.items():
    ...
    with io.open(fp, 'r', encoding='utf-8') as f:
        src = f.read()
    add = block(page, title, page, timeline)
    if not add:
        continue
    idx = src.rstrip().rfind('});')
    assert idx > 0, fp
    new = src[:idx] + add + src[idx:]
